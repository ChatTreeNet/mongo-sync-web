      ...
      <div className="logs-container">
        {groupedLogs.map((group, groupIndex) => {
          ...
          const stats = group.reduce((acc, log) => {
            ...
            if (log.message === 'Starting manual sync' || log.message === 'Starting scheduled sync') {
              const collections = log.details?.collections;
              if (typeof collections === 'string') {
                collections.split(',').forEach(c => acc.collections.add(c.trim()));
              }
            }
            return acc;
          ...
          return (
            <div key={groupIndex} className={`sync-group ${status}`}>
              <div className="sync-summary" onClick={() => toggleExpand(groupIndex)}>
